...
static int32_t validateScanPhysiNode(SScanPhysiNode* pNode, NodeValidationCtx* pCtx) {
  ...
  setValidateCol_IgnoreSlotId(pCtx, oldVal);
  if (TSDB_CODE_SUCCESS != code) {
    qLogWarn();
    return code;
  }
...
int32_t doValidateTableScanPhysiNode(STableScanPhysiNode* pNode) {
  ...
  setValidateCol_IgnoreSlotId(&ctx, oldVal);
  if (TSDB_CODE_SUCCESS != ctx.code) {
    qLogWarn();
    return ctx.code;
  }
...
int32_t doValidateProjectPhysiNode(SProjectPhysiNode* pNode) {
  ...
  setValidateCol_IgnoreOthers(&ctx, oldValIgnoreOthers);
  setValidateCol_ValidateFromDownstream(&ctx, oldVal);
  if (TSDB_CODE_SUCCESS != ctx.code) {
    qLogWarn();
    return ctx.code;
  }
